...
const TotalProperty = () => {
  ...
  return (
    <div className='md:p-10 p-4'>
      <div className='flex flex-col md:flex-row justify-around gap-3 md:gap-3'>
        {
            stats.map((stat)=>{
                return <Card key={stat.title} className="w-full dark:bg-gray-800 -space-y-5">
                    ...
                    <CardContent>
                        ...
                        <p className={`text-sx ${stat.trend === "up"? "text-green-500": "text-red-500"}`}>{stat.change}from last month</p>
                    </CardContent>
                </Card>
            })
        }
