...
const YourBlog = () => {
        ...
        {blog.map((item,index) => (
          <TableRow key={index}>
            <TableCell className="flex gap-4 items-center">
              ...
              <h1 onClick={()=>navigate(`/blogs/${item._id}`)} className='hover:underline cursor-pointer w-[55px] md:w-full truncate '>{item.title}</h1>
            </TableCell>
  ...
  <DropdownMenuContent>
      <DropdownMenuItem onClick={()=>navigate(`/deshboard/write-blog/${item._id}`)}><Edit/>Edit</DropdownMenuItem>
